...
import { Activity, AlertTriangle, ChevronLeft, Command, Gauge, LayoutDashboard, LifeBuoy, Menu, PanelLeftClose, Printer, Settings2, X } from 'lucide-react';
...
const navItems: NavItem[] = [
  ...
  { href: '/client/settings', labelAr: 'إعدادات الحساب', labelEn: 'Account settings', code: '03', icon: Settings2 },
];
...
export function DashboardSidebar({ currentPath, collapsed, onCollapse, mobileOpen, onMobileClose }: { currentPath: string; collapsed: boolean; onCollapse: () => void; mobileOpen: boolean; onMobileClose: () => void }) {
  ...
  const sidebarContent = <div className="flex h-full min-h-0 flex-col">
    <div className={`flex h-[78px] items-center border-b border-border/70 px-4 ${collapsed ? 'justify-center' : 'justify-between'}`}><Link href="/client" onClick={onMobileClose} aria-label="AJ—INDUSTRY client overview"><BrandMark compact={collapsed} /></Link>{!collapsed && <button type="button" onClick={onCollapse} className="grid size-8 place-items-center border border-border text-muted-foreground transition-colors hover:border-primary/60 hover:text-primary" aria-label={clientText(language, 'طي الشريط الجانبي', 'Collapse sidebar')}><PanelLeftClose className="size-4" /></button>}</div>
    ...
    <div className={`border-t border-border/70 p-4 ${collapsed ? 'flex justify-center' : ''}`}><div className={`flex items-center gap-2 ${collapsed ? 'justify-center' : 'justify-between'}`}>{!collapsed && <StatusLight label={clientText(language, 'متصل', 'ONLINE')} />}<button type="button" onClick={onMobileClose} className="text-muted-foreground transition-colors hover:text-primary" aria-label={clientText(language, 'مركز المساعدة', 'Help center')}><LifeBuoy className="size-4" /></button></div></div>
  </div>;

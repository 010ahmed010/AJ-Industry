...
export function DashboardSidebar({ currentPath, collapsed, onCollapse, mobileOpen, onMobileClose }: { currentPath: string; collapsed: boolean; onCollapse: () => void; mobileOpen: boolean; onMobileClose: () => void }) {
  ...
  const sidebarContent = <div className="flex h-full min-h-0 flex-col">
    <div className={`flex border-b border-border/70 ${collapsed ? 'h-[92px] flex-col items-center justify-center gap-2 px-2 py-2' : 'h-[78px] items-center justify-between px-4'}`}><Link href="/" onClick={onMobileClose} aria-label={clientText(language, 'العودة إلى الصفحة الرئيسية', 'Back to public home')}><BrandMark compact={collapsed} /></Link><button type="button" onClick={onCollapse} className="grid size-8 shrink-0 place-items-center border border-border bg-secondary/30 text-muted-foreground transition-colors hover:border-primary/60 hover:text-primary" aria-label={collapsed ? clientText(language, 'توسيع الشريط الجانبي (عرض الأسماء والأيقونات)', 'Expand sidebar (show icons & names)') : clientText(language, 'طي الشريط الجانبي (أيقونات فقط)', 'Collapse sidebar (icons only)')} title={collapsed ? clientText(language, 'توسيع الشريط الجانبي (عرض الأسماء والأيقونات)', 'Expand sidebar (show icons & names)') : clientText(language, 'طي الشريط الجانبي (أيقونات فقط)', 'Collapse sidebar (icons only)')}>{collapsed ? <PanelLeftOpen className="size-4 rtl:rotate-180" /> : <PanelLeftClose className="size-4 rtl:rotate-180" />}</button></div>
